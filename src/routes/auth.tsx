...
function AuthPage() {
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-background px-4 py-12"><div className="w-full max-w-md">
      <Link to="/welcome" className="flex items-center justify-center gap-2.5"><span className="brand-gradient grid size-9 place-items-center rounded-[9px] font-mono text-sm font-bold text-gold">IQ</span><span className="font-display text-xl tracking-tight">Certivo<span className="text-gold">IQ</span></span></Link>
      <div className="mt-7 rounded-xl border border-border bg-card p-6 shadow-sm"><h1 className="font-display text-[22px] leading-tight"><IQText>{heading}</IQText></h1><p className="mt-1 text-[13px] text-muted-foreground">{mfaMode ? "Enter the 6-digit code from your authenticator app." : mode === "forgot" ? "Enter your work email and we'll send a secure link to choose a new password." : "Your compliance workspace and CertivoIQ Academy."}</p>
