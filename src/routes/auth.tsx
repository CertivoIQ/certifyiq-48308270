...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!sessionWindow.valid) { await endExpiredSession(); return; }
      if (cancelled) return;
    ...
    })().catch(() => { if (!cancelled) toast.error("Could not verify your session. Please sign in again."); });
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        const { data, error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}${returnTo}`, data: { full_name: name, terms_version: BETA_TERMS_VERSION, terms_accepted_at: new Date().toISOString() } } });
        if (error) throw error;
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-background px-4 py-12"><div className="w-full max-w-md">
      <Link to="/welcome" className="flex items-center justify-center gap-2.5"><img src="/certivoiq-logo.png" alt="CertivoIQ" className="h-12 w-auto object-contain dark:hidden" /><img src="/certivoiq-logo-dark.png" alt="" aria-hidden="true" className="hidden h-12 w-auto object-contain dark:block" /></Link>
