...
function savePending(form: FormState) {
  if (typeof sessionStorage !== "undefined") sessionStorage.setItem(PENDING_KEY, JSON.stringify(form));
}
...
export function FreeReviewEntryGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!userEmail || lead.isLoading || lead.data || !pendingLead) return;
    setBusy(true);
  ...
  if (hasUser && lead.data) return <>{children}</>;
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const normalized: FreeReviewLeadInput = {
        ...
        properties: Number(form.properties),
        hq: form.hq.trim(),
        states: Array.isArray(form.states) ? form.states : splitList(String(form.states)),
        programs: Array.isArray(form.programs) ? form.programs : splitList(String(form.programs)),
      ...
      if (!hasUser) {
        ...
        const { data, error } = await supabase.auth.signUp({
          ...
          options: { emailRedirectTo: `${window.location.origin}/trial`, data: { full_name: form.fullName.trim() } },
        });
      ...
      const result = await captureLead({ data: normalized });
      clearPending();
    ...
    return (
      <Panel title="Check your email to continue" description="One quick verification step protects your workspace and keeps your FREE review history tied to your account." bodyClassName="p-5">
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-4">
          <div className="flex items-start gap-3"><Mail className="mt-0.5 size-5 shrink-0 text-primary" /><p className="text-sm leading-6">{message}</p></div>
        </div>
  ...
  return (
    <Panel
      ...
      description="Create your CertivoIQ account, tell us about your portfolio, and we'll prepare your 3 FREE certification reviews. Your certification upload comes immediately after this step."
      bodyClassName="p-5"
    >
      <div className="mb-5 flex items-start gap-3 rounded-lg border border-primary/20 bg-primary/5 p-4">
        <ShieldCheck className="mt-0.5 size-5 shrink-0 text-primary" />
        <div><p className="font-medium">Required before any FREE certification upload</p><p className="mt-1 text-sm text-muted-foreground">These details create your warm CRM lead, determine the best-fit plan, and provide the recipient fields needed for future email outreach.</p></div>
      </div>

      <form onSubmit={submit} className="grid gap-4 lg:grid-cols-2">
        {!hasUser && <>
          <div className="lg:col-span-2"><Label>Full name *</Label><Input required value={form.fullName} onChange={(e) => update("fullName", e.target.value)} placeholder="Your full name" /></div>
          <div><Label>Work email *</Label><Input required type="email" value={form.email} onChange={(e) => update("email", e.target.value)} placeholder="name@company.com" /></div>
          <div><Label>Password *</Label><Input required minLength={8} type="password" value={form.password} onChange={(e) => update("password", e.target.value)} placeholder="At least 8 characters" /></div>
        </>}
        ...
        <div className="lg:col-span-2 rounded-lg border bg-muted/30 p-4"><label className="flex items-start gap-3 text-sm leading-5"><input type="checkbox" required checked={form.marketingConsent} onChange={(e) => update("marketingConsent", e.target.checked)} className="mt-1 size-4 rounded border" /><span>I agree to receive CertivoIQ emails about my FREE reviews, relevant compliance updates, and plan recommendations. I can unsubscribe from marketing emails at any time.</span></label></div>
