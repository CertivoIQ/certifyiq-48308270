...
export function StateDocumentLibrary(){
 ...
 return <Panel className="mb-6" title="State document libraries" description="Separate libraries for all 50 states and the District of Columbia, with program checklists and agency sources.">
  <div className="space-y-5 p-4">
   ...
   <details className="rounded-lg border p-3"><summary className="cursor-pointer font-medium">{library.name} agency forms and source directory</summary><p className="my-3 text-sm text-muted-foreground">These are agency source references, primarily for LIHTC. Their current form revisions and program-specific requirements have not all been validated. HOME may be administered by a city or county; PHA requirements come from the actual PHA’s Administrative Plan or ACOP.</p>{library.agencies.map(a=><div key={`${a.name}-${a.scope}`} className="my-4"><h4 className="font-medium">{a.name}</h4><ul className="mt-2 space-y-2">{a.sources.map(s=><li key={s.url}><a className="text-sm underline" href={s.url} target="_blank" rel="noreferrer">{s.type.replaceAll('_',' ').toLowerCase()}</a></li>)}</ul></div>)}</details>
