...
import { filterTrainingLessons, parseTrainingProgress, type TrainingScope } from "@/lib/training/catalog";

export function TrainingCenter({ accountId, isStaff, scope }: { accountId: string | null; isStaff: boolean; scope: TrainingScope }) {
  const [query, setQuery] = useState("");
  const [audience, setAudience] = useState(scope === "pha" ? "pha" : "all");
  const [category, setCategory] = useState("all");
  const [selected, setSelected] = useState("training");
  const [completed, setCompleted] = useState<string[]>([]);
  const [storageReady, setStorageReady] = useState(false);
  const [storageError, setStorageError] = useState(false);
  const storageKey = accountId ? `certivoiq:training:v1:${scope}:${accountId}` : null;
  const available = useMemo(() => filterTrainingLessons(isStaff, "", scope === "pha" ? "pha" : "all", "all", scope), [isStaff, scope]);
  const lessons = useMemo(() => filterTrainingLessons(isStaff, query, audience, category, scope), [isStaff, query, audience, category, scope]);
  ...
  return <div className="space-y-5">
    <section className="rounded-xl border border-border bg-card p-5 sm:p-6" aria-label="Learning overview">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-2xl"><BookOpen className="mb-3 size-6 text-primary" aria-hidden="true" />
          <h2 className="text-xl font-semibold">{scope === "pha" ? "PHA role training" : "Your guide to staying audit ready"}</h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">{scope === "pha" ? "Training for PHA workflows is kept inside the PHA workspace and follows the pages available to your agency role." : "Start with setup, then follow the pages and tabs used in your daily work. PHA workflows are intentionally excluded from customer training."}</p>
        </div>
        <div className="min-w-48" aria-live="polite">
          ...
          <p className="mt-2 max-w-64 text-xs text-muted-foreground">{storageError ? "Device storage is unavailable. Progress is kept for this visit only." : accountId ? "Progress saved on this device for your account." : "Sign in to save progress on this device."}</p>
        </div>
      </div>
    </section>
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[2fr_1fr_1fr]" aria-label="Find training">
      <label className="text-sm font-medium">Search pages, tabs, or tasks
        <div className="relative mt-1"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" aria-hidden="true" /><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={scope === "pha" ? "Try portability, HUD-50058, or inspections" : "Try Jobs & Pay, notices, or CSV"} /></div>
      </label>
      <label className="text-sm font-medium">Workspace
        <select className={selectClass} value={audience} onChange={(event) => { setAudience(event.target.value); setCategory("all"); }}>
          {scope === "pha" ? <><option value="pha">PHA & shared guides</option></> : <><option value="all">All customer guides</option><option value="multifamily">Multifamily</option>{isStaff ? <option value="staff">Staff administration</option> : null}</>}
        </select>
      </label>
