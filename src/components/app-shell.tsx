...
function phaNavAllowed(role: PhaAgencyRole | null, key: (typeof PHA_NAV)[number]["key"]) {
  ...
  if (role === "executive") return ["family_read", "waiting_lists", "compliance", "findings"].includes(key);
  if (role === "inspection_staff") return ["inspections", "findings"].includes(key);
  if (role === "hcv_pbv_specialist") return ["family_write", "family_read", "hcv_operations", "waiting_lists", "compliance", "findings"].includes(key);
  if (role === "public_housing_specialist") return ["family_write", "family_read", "waiting_lists", "compliance", "findings"].includes(key);
  return false;
}
...
export function AppShell({ children, title, subtitle, actions }: { children: ReactNode; title: string; subtitle?: string; actions?: ReactNode }) {
  ...
  const isPha = profile.organization_type === "pha";
  const programSummary = isPha ? (profile.pha_programs.length ? profile.pha_programs.map((p) => p.toUpperCase()).join(" · ") : "PHA programs not configured") : (profile.selected_programs.length ? profile.selected_programs.map((p) => p.replaceAll("_", " ").toUpperCase()).join(" · ") : "Programs not configured");
  return (
