...
function phaNavAllowed(role: PhaAgencyRole | null, key: (typeof PHA_NAV)[number]["key"]) {
  ...
  if (role === "executive") return ["family_read", "waiting_lists", "accommodations", "compliance", "findings"].includes(key);
  if (role === "inspection_staff") return ["inspections", "accommodations", "findings"].includes(key);
  if (role === "hcv_pbv_specialist") return ["family_write", "family_read", "hcv_operations", "waiting_lists", "accommodations", "compliance", "findings"].includes(key);
  if (role === "public_housing_specialist") return ["family_write", "family_read", "waiting_lists", "accommodations", "compliance", "findings"].includes(key);
  return false;
}
...
function NavLinks({ onNavigate }: { onNavigate?: () => void }) {
  const { isStaff } = useIsStaff(); const { profile, phaRole } = useWorkspaceProfile(); const isPha = profile.organization_type === "pha"; const phaItems = PHA_NAV.filter((item) => phaNavAllowed(phaRole, item.key)); const items = [...(isPha ? phaItems : MULTIFAMILY_NAV), ...(isStaff ? [{ to: "/crm", label: "CRM", icon: Briefcase } as const] : [])];
