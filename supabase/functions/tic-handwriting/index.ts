...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(key){try{const response=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',signal:AbortSignal.timeout(90_000),headers:{authorization:'Bearer '+key,'content-type':'application/json'},body:JSON.stringify({model:MODEL,messages:[{role:'user',content:[{type:'text',text:transcriptionPrompt()},{type:'image_url',image_url:{url:body.image}}]}],response_format:{type:'json_object'},temperature:0,reasoning_effort:'none',max_completion_tokens:4500})});providerStatus=response.status;retryAfter=Math.min(60,Math.max(5,Number(response.headers.get('retry-after'))||30));if(response.ok){const result=await response.json();content=result.choices?.[0]?.message?.content??'';finishReason=String(result.choices?.[0]?.finish_reason??'');completionTokens=Number.isInteger(result.usage?.completion_tokens)?result.usage.completion_tokens:null;if(finishReason!=='stop')content='';}else await response.body?.cancel();}catch{providerStatus=503;}}
