...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(!response.ok){await response.body?.cancel();await finish(response.status===429?'rate_limited':'failed',{provider_status:response.status});return json({error:response.status===429?'Groq handwriting capacity is temporarily full. Retry this extraction shortly; no partial TIC was accepted.':'The handwriting provider could not read this page. Retry or confirm the fields manually.',providerStatus:response.status,retryAfter:Math.min(60,Math.max(5,Number(response.headers.get('retry-after'))||30))},response.status===429?429:502);}
  const result=await response.json();
  console.info(JSON.stringify({event:'tic_handwriting_response',page:body.page,finishReason:result.choices?.[0]?.finish_reason,completionTokens:result.usage?.completion_tokens,totalTokens:result.usage?.total_tokens}));
  if(result.choices?.[0]?.finish_reason!=='stop'){await finish('failed',{failure_code:'INCOMPLETE_TRANSCRIPTION',finish_reason:String(result.choices?.[0]?.finish_reason??'missing').slice(0,32),completion_tokens:Number.isInteger(result.usage?.completion_tokens)?result.usage.completion_tokens:null});finish=undefined;throw Error('INCOMPLETE_TRANSCRIPTION');}
  const transcription=validateTranscription(JSON.parse(result.choices[0].message.content),body.width,body.height);
  await finish('succeeded',{finish_reason:'stop',completion_tokens:Number.isInteger(result.usage?.completion_tokens)?result.usage.completion_tokens:null,field_count:transcription.fieldCount,unresolved_count:transcription.unresolvedCount});
