...
export function transcriptionPrompt(){return [
 ...
 'Return compact JSON {"fields":[["field_key","exact visible value",[x0,y0,x1,y1]]],"unreadable":["field keys with unclear filled cells"]}. Each field is a three-item array, not an object. Coordinates must surround the value cell in normalized image units 0 to 1000. Currency values use visible digits and optional decimal/comma/$, or the visible word None/Nil/Zero; dates use MM/DD/YYYY. Put uncertain entries only in unreadable. Return only keys in this catalog, expanding {row} where needed. No prose or markdown. Maximum 180 fields.',
 fieldCatalog()
 ].join('\n');}
function valueFor(field,raw){
 if(typeof raw!=='string'||!raw.trim()||raw.length>240||/[\r\n\u0000-\u001f]/.test(raw)||raw.includes('__CERTIVOIQ_'))return null;
 const v=raw.trim();
 if(field.type==='currency'&&/^(none|nil|zero)$/i.test(v))return '0';
...
export function validateTranscription(result,width,height){
 ...
 for(const item of result.fields){
  const f=Array.isArray(item)?{key:item[0],value:item[1],box:item[2],uncertain:item.length!==3}:item;
  ...
  accepted.set(f.key,{key:f.key,value,rawValue:f.value.trim(),bbox:{x0:b[0]*width/1000,y0:b[1]*height/1000,x1:b[2]*width/1000,y1:b[3]*height/1000}});
 }
 ...
 const lines=['Tenant Income Certification','__CERTIVOIQ_TIC_CELL_MODE__: strict',...proposals.flatMap(f=>[
  ...
  `__CERTIVOIQ_TIC_CELL__ ${f.key}: ${JSON.stringify({bbox:f.bbox,confidence:0.7,method:'vision-cell-proposal',rawValue:f.rawValue})}`
