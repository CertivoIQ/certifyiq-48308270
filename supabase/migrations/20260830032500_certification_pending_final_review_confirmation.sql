...
alter table public.certification_workflow_cases
  drop constraint if exists certification_workflow_cases_status_check;
alter table public.certification_workflow_cases
  add constraint certification_workflow_cases_status_check
  check (status in (
    'processing',
    'findings_assigned',
    'rerun_queued',
...
revoke all on function public.block_certification_final_review_confirmation_mutation()
  from public, anon, authenticated;
grant execute on function public.block_certification_final_review_confirmation_mutation()
  to service_role;

drop trigger if exists block_certification_final_review_confirmation_mutation
  on public.certification_final_review_confirmations;
...
-- Existing cases that have reached manager approval but have not been filed are now
-- explicitly pending final review. Findings remain a prerequisite.
...
-- Disable the legacy no-signature path so stale clients fail closed.
...
begin
  ...
  if lower(_signed) <> lower(_name) then
    raise exception 'typed signature must match the responsible party name';
  end if;
  ...
  if _case.status <> 'pending_final_review' then
    raise exception 'certification is not pending final review';
  end if;
  if exists (
    select 1 from public.certification_finding_assignments a
    where a.case_id = _case.id and a.status <> 'resolved'
  ...
  insert into public.certification_final_review_confirmations(
    case_id, certification_item_id, workspace_user_id,
    responsible_party_id, responsible_party_name, responsible_party_position,
    signature_text, attestation, confirmation_sha256, signed_at
  )
  values (
    _case.id,_item.id,_case.workspace_user_id,
    _actor,_name,_position,_signed,_attestation,_confirmation_sha,now()
  )
  ...
  values (
    ...
    'APPROVED',
    coalesce((select max(f.engine_build) from public.compliance_findings f where f.item_id = _item.id),'certivoiq-certification-workflow-v2'),
    _snapshot,
    _manifest_sha
  )
