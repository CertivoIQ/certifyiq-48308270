-- Batch 3: secret-authenticated, allowlisted HUD source evidence staging.
-- This RPC cannot activate compliance rules or send communications.
...
begin
  ...
  ) then raise exception 'invalid dataset row'; end if;
  ...
  ) on conflict (official_url, sha256) do nothing
  returning id into source_id;

  if source_id is not null then
    inserted := true;
  else
    select id into source_id from public.operations_source_versions
    where official_url = source_url and sha256 = source_sha256;
  end if;
  ...
  insert into public.operations_audit_events(
    ...
    'worker', case when inserted then 'source.staged' else 'source.unchanged' end,
    'operations_source_version', source_id::text, source_sha256,
    jsonb_build_array(jsonb_build_object('source_version_id', source_id)),
    jsonb_build_array(source_url), correlation,
    jsonb_build_object(
      'parser_build', parser_build,
...
revoke all on function public.operations_stage_hud_source_v1(text,text,text,text,timestamptz,text,bigint,text,jsonb) from public;
grant execute on function public.operations_stage_hud_source_v1(text,text,text,text,timestamptz,text,bigint,text,jsonb) to anon, service_role;

comment on function public.operations_stage_hud_source_v1(text,text,text,text,timestamptz,text,bigint,text,jsonb) is
  'Stages validated HUD schedule evidence only. It cannot activate compliance rules or communications.';
