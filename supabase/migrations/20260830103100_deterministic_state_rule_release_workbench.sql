...
-- Source validation, deterministic extraction/testing, independent release approval,
-- and certification Final Review are separate controls.

create table public.state_rule_release_work_items (
  pack_candidate_id uuid primary key references public.state_rule_pack_candidates(id) on delete restrict,
  ...
  blockers jsonb not null default '{}'::jsonb
    check (jsonb_typeof(blockers) = 'object'),
  updated_at timestamptz not null default now()
);

create table public.state_rule_source_extraction_assessments (
...
create index state_rule_source_extraction_latest_idx
  on public.state_rule_source_extraction_assessments(source_candidate_id, created_at desc, id desc);

create table public.state_rule_deterministic_rules (
  id uuid primary key default gen_random_uuid(),
...
create table public.state_rule_test_fixtures (
  ...
  fixture_kind text not null check (fixture_kind in (
    'POSITIVE','NEGATIVE','BOUNDARY','LAYERED_PROGRAM','SUPERSESSION'
  )),
...
create index state_rule_test_fixtures_latest_idx
  on public.state_rule_test_fixtures(pack_candidate_id, fixture_key, created_at desc, id desc);
create index state_rule_test_fixtures_rule_kind_idx
  on public.state_rule_test_fixtures(rule_id, fixture_kind, passed, created_at desc);

create table public.state_rule_source_conflicts (
  id uuid primary key default gen_random_uuid(),
  pack_candidate_id uuid not null references public.state_rule_pack_candidates(id) on delete restrict,
  ...
  conflict_type text not null check (conflict_type in (
    'CURRENCY','SUPERSESSION','SUBSTANTIVE','APPLICABILITY'
  )),
  ...
  check (
    ...
    or (
      nullif(btrim(resolution_citation),'') is not null
      and resolved_by is not null
      and resolved_at is not null
    )
  )
);
create index state_rule_source_conflicts_latest_idx
  on public.state_rule_source_conflicts(pack_candidate_id, conflict_key, created_at desc, id desc);

create table public.state_rule_pack_release_assessments (
  id uuid primary key default gen_random_uuid(),
  pack_candidate_id uuid not null references public.state_rule_pack_candidates(id) on delete restrict,
  source_snapshot_sha256 text not null check (source_snapshot_sha256 ~ '^[0-9a-f]{64}$'),
...
create index state_rule_pack_release_assessments_latest_idx
  on public.state_rule_pack_release_assessments(pack_candidate_id, source_snapshot_sha256, created_at desc, id desc);

create table public.state_rule_release_approval_events (
  id uuid primary key default gen_random_uuid(),
  pack_candidate_id uuid not null references public.state_rule_pack_candidates(id) on delete restrict,
  source_snapshot_sha256 text not null check (source_snapshot_sha256 ~ '^[0-9a-f]{64}$'),
...
create index state_rule_release_approval_events_latest_idx
  on public.state_rule_release_approval_events(pack_candidate_id, source_snapshot_sha256, created_at desc, id desc);

-- Controlled evidence is never browser-writable.
do $$
declare
  object_name text;
begin
...
-- Only the workbench finalizer may create validated state-pack releases.
revoke insert, update, delete, truncate, references, trigger
  on table public.state_rule_pack_releases from service_role;
grant select on table public.state_rule_pack_releases to service_role;

create or replace function public.enforce_deterministic_state_rule_release()
returns trigger
language plpgsql
set search_path = pg_catalog, public
as $$
begin
  if new.status = 'validated'::public.coverage_status
     and coalesce(new.release_evidence->>'control','') <> 'deterministic_state_rule_workbench' then
    raise exception 'Validated state-rule releases must come from the deterministic workbench';
  end if;
  return new;
end;
$$;
revoke all on function public.enforce_deterministic_state_rule_release() from public, anon, authenticated, service_role;

drop trigger if exists enforce_deterministic_state_rule_release on public.state_rule_pack_releases;
create trigger enforce_deterministic_state_rule_release
before insert or update on public.state_rule_pack_releases
for each row execute function public.enforce_deterministic_state_rule_release();

-- Exact current state sources plus the current shared federal source set.
...
declare
  ...
  v_unresolved_conflict_count integer := 0;
...
begin
  ...
  select coalesce(jsonb_agg(gap order by gap), '[]'::jsonb)
  into v_gaps
  from jsonb_array_elements_text(
    coalesce(v_pack.candidate_manifest->'required_document_family_gaps','[]'::jsonb)
  ) as gaps(gap)
  where gap in (
    'COMPLIANCE_GUIDEBOOK','INCOME_LIMITS','RENT_LIMITS','UTILITY_ALLOWANCE','COMPLIANCE_FORMS'
  );
  ...
  with current_sources as (
    ...
    select distinct on (assessment.source_candidate_id) assessment.*
    from public.state_rule_source_extraction_assessments assessment
    order by assessment.source_candidate_id, assessment.created_at desc, assessment.id desc
  )
  ...
  where rule.pack_candidate_id = v_pack.id
    ...
    and exists (
      select 1
      from public.state_rule_release_current_sources(v_pack.id) source
  ...
  select count(*) filter (where not passed)::integer
  into v_failed_fixture_count
  from latest_fixture;
  ...
  where rule.pack_candidate_id = v_pack.id
    ...
    and exists (
      select 1
      from unnest(array['POSITIVE','NEGATIVE','BOUNDARY']) as kinds(required_kind)
      where not exists (
        select 1
        from (
          select distinct on (fixture.fixture_key) fixture.*
  ...
  into v_unresolved_conflict_count
  ...
  if exists (
    ...
    v_status := 'RELEASED';
  elsif not coalesce(v_snapshot.sources_ready,false)
        or jsonb_array_length(v_gaps) > 0 then
    v_status := 'SOURCE_DOCUMENT_GAP';
  elsif v_source_count = 0
        or v_assessed_count <> v_source_count
        or v_assessment.id is null
        or not v_assessment.source_extraction_complete
        or v_rule_count = 0 then
    v_status := 'RULE_EXTRACTION';
  elsif v_failed_fixture_count > 0
        or v_rules_missing_core_fixtures > 0
        or not v_layered_ok
        or not v_supersession_ok then
    v_status := 'FIXTURE_VALIDATION';
  elsif not v_assessment.conflict_inventory_complete
        or v_unresolved_conflict_count > 0 then
    v_status := 'CONFLICT_VALIDATION';
  else
    v_status := 'READY_FOR_INDEPENDENT_RELEASE';
  end if;

  v_blockers := jsonb_build_object(
    'sources_ready', coalesce(v_snapshot.sources_ready,false),
    'release_critical_document_gaps', v_gaps,
    'current_source_count', v_source_count,
    'assessed_source_count', v_assessed_count,
    ...
    'source_extraction_complete', coalesce(v_assessment.source_extraction_complete,false),
    'conflict_inventory_complete', coalesce(v_assessment.conflict_inventory_complete,false),
    'unresolved_conflict_count', v_unresolved_conflict_count
  );

  insert into public.state_rule_release_work_items(
    pack_candidate_id,state_code,source_snapshot_sha256,status,
    release_critical_document_gaps,blockers,updated_at
  ) values (
    v_pack.id,v_pack.state_code,v_snapshot.source_snapshot_sha256,v_status,
    v_gaps,v_blockers,now()
  )
...
-- Retire the legacy caller-supplied counts/hashes release function.
...
begin
  ...
  if nullif(btrim(coalesce(p_notes,'')),'') is null then
    raise exception 'release review notes are required';
  end if;
  if not exists (
    select 1
  ...
  if v_actor = v_source_approval.activator_id
     or v_actor = any(v_source_approval.first_reviewer_ids) then
...
begin
  ...
  limit 1;
  if v_source_approval.id is null then
    raise exception 'current source snapshot approval is required';
  end if;
  ...
  select encode(extensions.digest(convert_to(
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'state',source_state_code,
      'type',source_type,
      'url',source_url,
      'sha256',source_sha256,
      'retrieved_at',retrieved_at
    ) order by source_candidate_id),'[]'::jsonb)::text,
    'UTF8'
  ),'sha256'),'hex')
  ...
  select
    ...
    encode(extensions.digest(convert_to(
      coalesce(jsonb_agg(jsonb_build_object(
        ...
        'source',rule.source_candidate_id,
        'source_sha256',rule.source_sha256,
        'page',rule.source_page,
        'citation',rule.citation,
        'operation_type',rule.operation_type,
        'input_schema',rule.input_schema,
        'operation',rule.deterministic_operation,
        'effective_from',rule.effective_from,
        'effective_to',rule.effective_to
      ) order by rule.rule_key,rule.rule_version,rule.id),'[]'::jsonb)::text,
      'UTF8'
    ),'sha256'),'hex')
  ...
  select
    ...
    encode(extensions.digest(convert_to(
      coalesce(jsonb_agg(jsonb_build_object(
        ...
        'passed',passed
      ) order by fixture_key),'[]'::jsonb)::text,
      'UTF8'
    ),'sha256'),'hex')
  ...
  select
    ...
    encode(extensions.digest(convert_to(
      coalesce(jsonb_agg(jsonb_build_object(
        ...
        'type',conflict_type,
        'sources',source_candidate_ids,
        'description',description,
        'status',resolution_status,
        'resolution_citation',resolution_citation,
        'resolved_by',resolved_by,
        'resolved_at',resolved_at
      ) order by conflict_key),'[]'::jsonb)::text,
      'UTF8'
    ),'sha256'),'hex')
  ...
  if v_rule_count <= 0
     or v_fixture_count < 5
     or v_failed_fixture_count <> 0
     or v_unresolved_conflict_count <> 0 then
  ...
  perform public.refresh_state_rule_release_work_item(v_pack.id);
...
$$;
revoke all on function public.finalize_state_rule_pack_release(uuid,text,date,text,text)
  from public, anon, authenticated;
grant execute on function public.finalize_state_rule_pack_release(uuid,text,date,text,text)
  to service_role;
...
as $$
  select
    ...
    work.state_code,
    work.status,
    work.release_critical_document_gaps,
    work.blockers,
    work.updated_at
...
-- Initialize 50 fail-closed release work items. No release is created here.
do $$
declare
  pack record;
begin
  for pack in
    select id from public.state_rule_pack_candidates where state_code <> 'US'
  loop
...
-- Production invariant: no active state without a workbench release.
do $$
declare
