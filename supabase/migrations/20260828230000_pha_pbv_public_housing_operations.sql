...
create table if not exists public.pha_pbv_hap_contracts (
  ...
  annual_anniversary date not null,
...
create table if not exists public.pha_pbv_rent_actions (
  id uuid primary key default gen_random_uuid(),
  hap_contract_id uuid not null references public.pha_pbv_hap_contracts(id) on delete cascade,
  ...
  hqs_compliant boolean not null default false,
  owner_request_received_at date, effective_date date,
...
create table if not exists public.pha_pbv_move_requests (
  ...
  assisted_lease_start date not null,
  request_received_at date not null default current_date,
  owner_notice_documented boolean not null default false,
  vawa_emergency_transfer boolean not null default false,
...
create table if not exists public.pha_public_housing_rent_elections (
  ...
  rent_option text not null check (rent_option in ('income_based','flat_rent')),
  income_based_rent numeric(12,2) not null check (income_based_rent >= 0),
  flat_rent numeric(12,2) not null check (flat_rent >= 0),
  selected_rent numeric(12,2) not null check (selected_rent >= 0),
  hardship_review_required boolean not null default false,
  notice_issued boolean not null default false,
  elected_at timestamptz not null default now(),
  unique(family_action_id,election_year)
);
...
create table if not exists public.pha_public_housing_over_income_cases (
  ...
  workspace_user_id uuid not null default public.current_pha_workspace_user_id() references auth.users(id) on delete cascade,
  family_action_id uuid not null references public.pha_family_actions(id) on delete cascade,
  over_income_limit numeric(12,2) not null check (over_income_limit >= 0),
  first_over_income_determination date not null,
  ...
  alternative_rent numeric(12,2),
  non_public_housing_lease_executed_at date,
...
alter table public.pha_pbv_hap_contracts enable row level security;
alter table public.pha_pbv_rent_actions enable row level security;
alter table public.pha_pbv_move_requests enable row level security;
alter table public.pha_public_housing_rent_elections enable row level security;
alter table public.pha_public_housing_over_income_cases enable row level security;
...
create or replace function public.prepare_pha_pbv_rent_action() returns trigger language plpgsql security invoker as $$
declare h public.pha_pbv_hap_contracts%rowtype;
begin
 select * into h from public.pha_pbv_hap_contracts where id=new.hap_contract_id;
 if not found then raise exception 'PBV HAP contract not found'; end if;
 if new.action_type in ('owner_request','ocaf') and not new.hqs_compliant then
   new.determination_status:='blocked'; new.block_reason:='PBV rent increase requires HAP contract compliance including HQS';
 end if;
 if new.determination_status='approved' then
   if new.approved_rent is null or new.reasonable_rent is null or new.applicable_cap is null then raise exception 'PBV approved rent requires approved rent, reasonable rent, and applicable cap'; end if;
   if new.approved_rent > least(new.reasonable_rent,new.applicable_cap) then raise exception 'PBV approved rent exceeds controlled rent limit'; end if;
   if not new.written_notice_issued then raise exception 'PBV rent change requires written notice to owner'; end if;
 end if;
...
create or replace function public.prepare_pha_pbv_move_request() returns trigger language plpgsql security invoker as $$
declare f public.pha_family_actions%rowtype;
begin
 select * into f from public.pha_family_actions where id=new.family_action_id;
 if not found or f.program_code <> 'pbv' then raise exception 'PBV move request requires a PBV family action'; end if;
 new.one_year_requirement_satisfied := new.request_received_at >= (new.assisted_lease_start + interval '1 year')::date;
 if not new.one_year_requirement_satisfied and not new.vawa_emergency_transfer then new.offer_status:='blocked'; end if;
 if new.offer_status in ('offered','completed') and new.assistance_offer_type is null then raise exception 'PBV move assistance offer type is required'; end if;
 new.updated_at:=now(); return new;
end; $$;
...
create or replace function public.prepare_pha_ph_over_income_case() returns trigger language plpgsql security invoker as $$
begin
 if new.consecutive_months >= 24 then
   new.notice_stage:='post_24_month';
   if new.pha_post_24_policy is null then new.status:='blocked';
   elsif new.pha_post_24_policy='alternative_non_public_housing_rent' then
     if new.alternative_rent is null then raise exception 'Alternative non-public housing rent is required'; end if;
     new.status:='alternative_rent';
   else new.status:='termination_pending'; end if;
 elsif new.consecutive_months >= 12 then new.notice_stage:='twelve_month';
 else new.notice_stage:='initial'; end if;
 new.updated_at:=now(); return new;
end; $$;
