DROP FUNCTION public.merlin_claim_native_preparation(text);
...
CREATE OR REPLACE FUNCTION public.merlin_capture_native_preparation(_job_id uuid,_worker text,_evidence jsonb)
RETURNS boolean LANGUAGE plpgsql SECURITY INVOKER SET search_path=pg_catalog AS $$
DECLARE j public.operations_jobs; d public.merlin_procedure_documents; c public.state_rule_source_candidates;
BEGIN
 IF current_user NOT IN ('postgres','service_role') THEN RAISE EXCEPTION 'service role required'; END IF;
 ...
 UPDATE public.merlin_procedure_documents SET
  source_snapshot=source_snapshot||jsonb_build_object('native_capture',_evidence||'{"ready_for_extraction":true}'::jsonb),
  crawl_attempts=crawl_attempts+1,last_crawled_at=now(),updated_at=now()
 WHERE id=d.id;
 INSERT INTO public.merlin_procedure_extraction_events(document_id,job_id,event_type,source_sha256,actor_kind,detail)
 VALUES(d.id,j.id,'source_verified',d.source_sha256,'worker',_evidence||'{"stage":"source_ready_not_extracted"}'::jsonb);
 UPDATE public.operations_jobs SET status='queued',scheduled_at='infinity'::timestamptz,
  result=jsonb_build_object('stage','source_ready_not_extracted','sha256',d.source_sha256),
  lease_owner=NULL,lease_expires_at=NULL,heartbeat_at=now(),updated_at=now()
 WHERE id=j.id;
 INSERT INTO public.operations_audit_events(actor_kind,action,target_type,target_id,correlation_id,detail)
 VALUES('worker','merlin.native.source_prepared','operations_job',j.id::text,j.correlation_id,
  '{"stage":"source_ready_not_extracted"}'::jsonb);
 RETURN true;
END $$;
REVOKE ALL ON FUNCTION public.merlin_capture_native_preparation(uuid,text,jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.merlin_capture_native_preparation(uuid,text,jsonb) TO service_role;
