...
test('income header hard-stops household mapping when PART III heading is missed', () => {
  const blocks = [block([
    ...
    // OCR missed the PART III title, but recognized the table header and values.
    incomeHeader,
    ...
    line('TOTALS $ 25,364.40 $ 0.00 $ 0.00 $ 0.00', 500, 522, [
      word('TOTALS', 35, 500, 95, 520), word('$', 150, 500, 160, 520), word('25,364.40', 180, 500, 260, 520), word('$', 390, 500, 400, 520), word('0.00', 430, 500, 470, 520),
    ]),
    line('PART IV - INCOME FROM ASSETS', 560, 580, [word('PART', 300, 560, 345, 580), word('IV', 350, 560, 375, 580), word('INCOME', 390, 560, 450, 580), word('ASSETS', 520, 560, 585, 580)]),
  ])];
  ...
  const lines = extractTicSpatialValueLines(blocks, 1000, 1000);
  assert(lines.includes('household member 1 last name: Mobley'));
  assert(lines.includes('income member 1 employment or wages: 25364.40'));
  assert(!lines.some((value) => /household member [2-9]/.test(value)), 'income labels/totals must never become household rows');
  assert(!lines.some((value) => value.includes('household member 8 last name: 25,364.40')));
  assert(!lines.some((value) => value.includes('household member 9')));
});
...
test('unrelated race text does not force PHFA household column geometry', () => {
  ...
  assert(lines.includes('household member 1 last name: Mobley'));
  assert(lines.includes('household member 1 date of birth: 01/17/1965'));
  assert(!lines.some((value) => value.includes('household member 1 race:')));
});
