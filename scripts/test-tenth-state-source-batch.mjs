import assert from"node:assert/strict";import{readFile}from"node:fs/promises";import test from"node:test";const d=JSON.parse(await readFile(new URL("../src/lib/tenth-state-source-batch-candidates.json",import.meta.url)));
test("contains KS IA IN IL once",()=>{assert.deepEqual(d.states.map(s=>s.state_code),["KS","IA","IN","IL"]);assert.equal(new Set(d.states.map(s=>s.state_code)).size,4)});
...
test("requires Indiana notices alongside the older manual",()=>{const i=d.states.find(s=>s.state_code==="IN");assert.ok(i.sources.some(x=>x.type==="REAL_ESTATE_DEPARTMENT_NOTICES"));assert.ok(i.conflicts.some(x=>x.includes("2021 compliance manual")))});
