...
test('prepare hashes the calculator page separately from its shared engine without rewriting public bytes',t=>{const {cwd,run}=fixture(t);assert.equal(run('identity').status,0);const out=join(cwd,'.output/public/assets');mkdirSync(out,{recursive:true});const marker=`income-calculator-release-${sha}.json`;copyFileSync(join(cwd,'public/assets',marker),join(out,marker));const original=readFileSync(join(out,marker));const bytes='export const title="Income Calculator";';writeFileSync(join(out,'income-calculator-abcdef.js'),bytes);writeFileSync(join(out,'income-calculator-engine-fedcba.js'),'export const evaluate=()=>0;');const result=run('prepare');assert.equal(result.status,0,result.stderr);assert.deepEqual(readFileSync(join(out,marker)),original);const evidence=JSON.parse(readFileSync(join(cwd,'income-build-verification.json')));assert.equal(evidence.assetSha256,createHash('sha256').update(bytes).digest('hex'));assert.equal(evidence.asset,'/assets/income-calculator-abcdef.js');});
