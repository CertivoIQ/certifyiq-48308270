...
try {
 ...
 await page.locator('input[type=file]').setInputFiles({name:'TEST-ONLY-acceptance-onboarding.csv',mimeType:'text/csv',buffer:Buffer.from(csv)});await page.getByRole('button',{name:'Import onboarding records',exact:true}).click();await page.getByRole('status').filter({hasText:'Created or updated 1 properties, 1 units, and 1 tenant profiles'}).waitFor();
