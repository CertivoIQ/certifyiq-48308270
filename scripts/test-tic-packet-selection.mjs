...
const db={
  ...
  from(table) { let operation='select', payload;const q={select(){return q},eq(){return q},order(){return q},limit(){return q},update(value){operation='update';payload=value;return q},delete(){operation='delete';return q},insert(value){operation='insert';payload=value;return q},
    maybeSingle:async()=>({data:table==='certification_import_jobs'?{id:'packet-job',status:'processing'}:table==='certification_import_items'?null:{id:tenantId,property_id:'property',unit_id:'unit',program_codes:['LIHTC'],portfolio_properties:{jurisdiction:'GA'}},error:null}),
