import assert from"node:assert/strict";import{readFile}from"node:fs/promises";import test from"node:test";const d=JSON.parse(await readFile(new URL("../src/lib/ninth-state-source-batch-candidates.json",import.meta.url)));
test("contains MD ME LA KY once",()=>{assert.deepEqual(d.states.map(s=>s.state_code),["MD","ME","LA","KY"]);assert.equal(new Set(d.states.map(s=>s.state_code)).size,4)});
...
test("blocks Maine forms while official page is under update",()=>{const me=d.states.find(s=>s.state_code==="ME");assert.ok(me.sources.some(x=>x.status==="BLOCKED_FORMS_PAGE_UNDER_UPDATE"))});
