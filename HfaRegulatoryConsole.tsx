        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-5 py-7 sm:px-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-cyan-700">
              ...
              <span className="text-sm font-semibold uppercase tracking-[.16em]">
                HFA Regulatory Console
              </span>
            </div>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">{agency.name}</h1>
            <p className="mt-2 text-sm text-slate-600">
              {agency.stateCode} · {agency.authorityScope.join(" · ")}
            </p>
          </div>
          <div className="rounded-xl bg-slate-950 px-4 py-3 text-sm text-white">
            <span className="font-semibold">Agency workspace</span>
            <span className="ml-2 text-slate-300">
              Only explicitly submitted records are visible.
            </span>
          </div>
        </div>
        <nav
          className="mx-auto flex max-w-7xl gap-1 overflow-x-auto px-5 sm:px-8"
          aria-label="Regulatory console sections"
        >
          {tabs.map((item) => (
            <button
              key={item.id}
              onClick={() => setTab(item.id)}
              className={`border-b-2 px-4 py-3 text-sm font-semibold whitespace-nowrap ${tab === item.id ? "border-cyan-600 text-cyan-800" : "border-transparent text-slate-500 hover:text-slate-900"}`}
            >
              {item.label}
            </button>
          ))}
        ...
        {tab === "overview" && (
          <section aria-labelledby="overview-title">
            ...
            <div className="mt-7 grid gap-6 lg:grid-cols-2">
              <Panel title="Submission readiness" icon={<ClipboardCheck className="h-5 w-5" />}>
        ...
        {tab === "submissions" && (
          <section>
            <SectionTitle
              ...
              description="Preflight results, evidence manifests and monitoring packages explicitly submitted to this agency."
            />
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold">
                        {release.stateCode} · {release.program} · {release.version}
                      </p>
                      <p className="mt-1 text-sm text-slate-600">
                        Effective {release.effectiveFrom} · {release.sourceCount} controlling
        ...
        {tab === "sampling" && (
          <section>
            <SectionTitle
              ...
              title="Risk-based monitoring sample"
              description="Create transparent random or risk-weighted samples without using protected characteristics."
            />
            {canRunSampling ? (
              ...
              <p className="mt-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">
                Your role cannot create monitoring samples.
              </p>
            )}
          </section>
        )}
        ...
        {tab === "corrections" && (
          <section>
            <SectionTitle
              ...
              title="Corrective Action Exchange"
              description="Track findings, owner responses, correction evidence, re-review and agency disposition."
            />
            <div className="mt-6 grid gap-4">
                ...
                <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5">
                  <div className="flex flex-wrap justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">
                        {item.organizationName} · {item.propertyName} · due {item.dueAt} ·{" "}
                        {item.evidenceCount} evidence item(s)
                      </p>
                    </div>
                    <span className={`${badge} bg-amber-50 text-amber-900 ring-amber-200`}>
...
function SubmissionList({ items }: { items: HfaSubmission[] }) {
  return (
      ...
      {items.map((item) => (
        ...
        >
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              {item.organizationName} · {item.program} · {item.reportingPeriod} ·{" "}
              {item.unresolvedFindings} unresolved
            </p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold">{item.readinessScore}% ready</span>
            <span className={`${badge} bg-slate-50 text-slate-700 ring-slate-200`}>
              {item.status.replaceAll("_", " ")}
            </span>
          </div>
        </article>
